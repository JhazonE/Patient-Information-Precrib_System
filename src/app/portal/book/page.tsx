...
function PhAddressSelector({
  ...
  const disabled: React.CSSProperties = { ...sel, opacity: 0.45, cursor: "not-allowed", background: "#f1f5f9" };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
      <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
        <label style={labelStyle}>House / Unit No. & Street</label>
        <input
          value={street}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
          ...
          <select value={regionCode} onChange={(e) => selectRegion(e.target.value)}
            style={loadingRegions ? disabled : sel} disabled={loadingRegions}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
          ...
          <select value={provinceCode} onChange={(e) => selectProvince(e.target.value)}
            style={!regionCode || loadingProvinces || provinces.length === 0 ? disabled : sel}
            disabled={!regionCode || loadingProvinces || provinces.length === 0}
            onFocus={handleFocus} onBlur={handleBlur}>
            <option value="">{loadingProvinces ? "Loading..." : !regionCode ? "Select region first" : provinces.length === 0 && regionCode ? "N/A" : "Select Province"}</option>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
          ...
          <select value={cityCode} onChange={(e) => selectCity(e.target.value)}
            style={(!regionCode && !provinceCode) || loadingCities || cities.length === 0 ? disabled : sel}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
          ...
          <select value={barangayCode} onChange={(e) => setBarangayCode(e.target.value)}
            style={!cityCode || loadingBarangays || barangays.length === 0 ? disabled : sel}
